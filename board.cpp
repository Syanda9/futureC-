...
board::board():woc(0),boc(0){
    ...
    for(int i=0;i<9;i++){
        if(i<5){
            rows.push_back(new cell[1+i]);
            connectHorizontal(rows[i],1+i);
            mapCells(rows[i],r--,1+i);
            if(i<3){
                setMarbles(rows[i],1+i,'O');
            }else
                setMarbles(rows[i],1+i,'+');
        }
        else{
            rows.push_back(new cell[3+6-i]);
            connectHorizontal(rows[i],3+6-i);
            mapCells(rows[i],r--,3+6-i);
            if(i>=6){
                setMarbles(rows[i],3+6-i,'@');
            }else
                setMarbles(rows[i],3+6-i,'+');
        }
    }
    for(int i=1;i<9;i++){
        if(i<5){
            connectDiagonal(rows[i],rows[i-1],0,1+i-1,true);
            connectDiagonal(rows[i],rows[i-1],1,1+i-1,true);
        }
        else{
            connectDiagonal(rows[i-1],rows[i],0,3+6-i,false);
            connectDiagonal(rows[i-1],rows[i],1,3+6-i,false);
        }
    }
